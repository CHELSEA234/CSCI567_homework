...
def logistic_train_ovr(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    for i in range(max_iterations):
        ...
        h_x = sigmoid(numpy.matmul(Xtrain, w) + b )                  # why is there different with Matlab in coursra
...
def logistic_test_ovr(Xtest, w_l, b_l):
    ...
    test_pred = sigmoid(numpy.matmul(Xtest, w_l) + b_l)
...
def logistic_mul_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    for i in range(max_iterations):
        result = softmax (numpy.matmul(Xtrain, w.transpose()) + b )  
        cost = -1 * numpy.mean(ytrain_mat * numpy.log(result))  
        grad_w = -1 * numpy.matmul((result - ytrain_mat).transpose(), Xtrain )/sample_num
        grad_b = -1 * numpy.mean((result - ytrain_mat).transpose(), 1)
        w = w + step_size * grad_w
        b = b + step_size * grad_b
...
def logistic_mul_test(Xtest, w_l, b_l):
    ...
    """
    # you need to fill in your solution here
    test_pred = softmax (numpy.matmul(Xtest, w_l.transpose()) + b_l )
...
def softmax(x):
    for i in range(len(x)):
        score = x[i]
        x[i] = numpy.exp(score) / numpy.sum(numpy.exp(score), axis=0)
    return x
