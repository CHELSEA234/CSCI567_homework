...
def nb_train_smoothing(X, Y, vocab_size, num_classes, alpha):
    ...
    class_prob[0] = num_0 * 1.0/ num
    class_prob[1] = num_1 * 1.0/ num


    # making class_word_prob 
    num_train_email = len(X)

    for j in range(num_train_email):
        for i, (index, frequency) in enumerate(X[j]):
            class_word_prob[Y[j], index] = class_word_prob[Y[j], index] + frequency

    class_word_sum = np.sum(class_word_prob, axis = 1)
    class_word_prob[0] = class_word_prob[0] / class_word_sum[0]
    class_word_prob[1] = class_word_prob[1] / class_word_sum[1]
...
def nb_predict(X, class_prob, class_word_prob):
    ...
    # Q8.1 Edit here
    ###################################################
    num_test_email = len(X)

    for j in range(num_test_email):
        
        ham_1 = np.log(class_prob[0])

        ham_2 = 0
        for i, (index, frequency) in enumerate(X[j]):
            pro_word_class = class_word_prob[0, index]
            if pro_word_class != 0:
                cur = frequency * np.log(pro_word_class)
            elif class_word_prob[1, index] != 0:
                cur = frequency * (-1e50)
            else:
                cur = 0     
            ham_2 = ham_2 + cur

        ham_loglikelihood = ham_1 + ham_2


        spam_1 = np.log(class_prob[1])

        spam_2 = 0

        for i, (index, frequency) in enumerate(X[j]):
            pro_word_class = class_word_prob[1, index]
            if pro_word_class != 0:
                cur = frequency * np.log(pro_word_class)
            elif class_word_prob[0, index] != 0:
                cur = frequency * (-1e50)
            else:
                cur = 0     
            spam_2 = spam_2 + cur

        spam_loglikelihood = spam_1 + spam_2


        if spam_loglikelihood >= ham_loglikelihood:     # this is spam mail
            Ypred.append(1)
        else:
            Ypred.append(0)
